...
# iri_mo_predicted <- as.character(
#   month(iri_pub_date+ months(parse_number(names(r_iri_lts))),label=T))
# r_iri_lts %>% 
#   set.names(iri_mo_predicted)
...
# resample composite IRI to crop phenology resolution (1 km)
# Note - default sampling will use bilinear interpolation which will 
# affect visualization - should discuss
r_iri_composite_1km <- resample(r_iri_composite, r_phen_m3_crop)

# resamle individual IRI lts to crop phenology resolution (1km)
# r_iri_lts_1km <- resample(r_iri_lts, r_phen_m4_crop)
...
# this doesn't really make sense anymroe
r_iri_composite_masked <-  mask(r_iri_composite_1km, r_phen_m3_crop)
...
r_iri_composite_masked <-  mask(r_iri_composite_1km, mask_phen_m4)
...
# Composite 4 month map

```{r}
r_iri_masked_cropped <-  crop(r_iri_composite_masked, gdf_aoi)
r_iri_masked_clipped <-  mask(r_iri_masked_cropped, gdf_aoi)


p_map_composite_4 <- ggplot()+
  geom_spatraster(
...
lr_iri_masked_clipped <- lr_iri_lts_masked %>% 
  map(
    \(r){
      r_cropped <- crop(r, gdf_aoi)
      r_clipped <- mask(r_cropped, gdf_aoi)
      return(r_clipped  )
    }
  )

# map through
p_map_indiv_mo <- lr_iri_masked_clipped %>% 
  map(\(r){
  mo_predicted <- names(r)
  # hacky since i don't want to process the rasters right now:
  mo_yr_predicted<- ifelse(mo_predicted!="Jan",paste0("2023 ", mo_predicted),paste0("2024 ",mo_predicted))
  ...
  # theme_void()+
  labs(
    title = glue("Seasonal below average rainfall forecast probability in\nareas with an active season {mo_yr_predicted}"),
    subtitle = glue("Date forecast: 2023 September\nPrediction: {mo_yr_predicted}"),
    caption = "Seasonal forecast: IRI\nSeasonal Calendar: ASAP\nActive season layer created by compositing all areas classified as 'active' over the month"
  )+
  theme(
    panel.grid = element_blank(),
    axis.ticks = element_blank(),
...
```{r}
# try the above but faceted to get the benefits of facet (i.e not multiple legend/titles)
# lr_iri_lts_masked
...
# hacky reset names:
# names_new <- ifelse(names(r_iri_lts_masked_clipped)!="Jan",
#        paste0(names(r_iri_lts_masked_clipped)," 2023"),
#        paste0(names(r_iri_lts_masked_clipped)," 2024"))
# 
# r_iri_lts_masked_clipped %>% 
#   set.names(
#     names_new
#   )
#     
